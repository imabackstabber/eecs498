...
def make_adversarial_attack(X, target_y, model, max_iter=100, verbose=True):
  ...
  for i in range(max_iter):
      score = model(X_adv)
      correct_class = torch.argmax(score,dim=1)
      
      if correct_class == target_y:
          break
      score[:,target_y].backward()
      print('iter:{},loss:{}',i,score[:,target_y])
      g = X_adv.grad.data
      dX = learning_rate * g / torch.sum(torch.square(g))
      X_adv.data += dX.data # gradient ascent
...
def class_visualization_step(img, target_y, model, **kwargs):
    ...
    # Replace "pass" statement with your code
    score = model(img)
    final_score = score[:,target_y] - l2_reg*torch.norm(img,2)
    final_score.backward()
    g = img.grad.data
    img.data += learning_rate * g # gradient ascent
    img.grad.data.zero_()
    ########################################################################
    #                             END OF YOUR CODE                         #
    ########################################################################
