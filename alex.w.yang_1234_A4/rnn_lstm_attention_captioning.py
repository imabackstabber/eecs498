...
def rnn_step_backward(dnext_h, cache):
    ...
    dout = dnext_h * (1 - next_h ** 2) # tanh
    dx = torch.mm(dout,Wx.t())
    dprev_h = torch.mm(dout,Wh.t())
    dWx = torch.mm(x.t(),dout)
    dWh = torch.mm(prev_h.t(),dout)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    cache = []
    for i in range(T):
        hidden,ca = rnn_step_forward(x[:,i,:], hidden, Wx, Wh, b)
        h[:,i,:] = hidden
        cache.append(ca)
    ##############################################################################
    #                               END OF YOUR CODE                             #
...
def rnn_backward(dh, cache):
    ...
    dh0 = torch.zeros((N, H), dtype=cache[0][2].dtype, device=cache[0][2].device)
...
def temporal_softmax_loss(x, y, ignore_index=None):
    ...
    # Replace "pass" statement with your code
    N,T,V = x.shape
    loss = torch.div(torch.nn.functional.cross_entropy(x.reshape(N*T,V),y.reshape(N*T),ignore_index = ignore_index,reduction='sum'),N)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(self, word_to_idx, input_dim=512, wordvec_dim=128,
        ...
        if cell_type == 'rnn' or cell_type == 'lstm':
            self.feat_extract = FeatureExtractor(pooling = True,dtype = dtype,device = device)
            if cell_type == 'rnn':
            ...
            self.feat_extract = FeatureExtractor(pooling = False,dtype = dtype,device = device)
        ...
        self.result_affine = nn.Linear(hidden_dim,vocab_size).to(device=device, dtype=dtype)
        nn.init.kaiming_normal_(self.result_affine.weight)
        nn.init.zeros_(self.result_affine.bias)
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
    ...
    def sample(self, images, max_length=15):
        ...
        if self.cell_type == 'attention':
          feature = feature.permute(0, 2, 3, 1) # make it N * 4 * 4 * input_dim
        prev_h = self.affine(feature) # input_dim -> hidden_dim
        prev_c = torch.zeros_like(prev_h) # for lstm
        if self.cell_type == 'attention':
        ...
        # self._start -> vocab_size , word_embed(self._start) -> wordvec_dim,but need reshape(1,-1) to broadcast from (wordvec_dim,) to 
        # (1,wordvec_dim)
        for i in range(max_length):
          ...
          score = self.result_affine(next_h) # from hidden_dim to vocab size
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b, attn=None, Wattn=None):
    ...
    H = prev_h.shape[1]
    if attn is not None:
        activation = torch.mm(x,Wx) + torch.mm(prev_h,Wh) + torch.mm(attn,Wattn) + b
    else:
        activation = torch.mm(x,Wx) + torch.mm(prev_h,Wh) + b
    ai = activation[:,:H]
    af = activation[:,H:2*H]
...
def dot_product_attention(prev_h, A):
    ...
    A_tilt = A.reshape(N,H,-1) # (N,H,16)
    M_attn = (torch.bmm(h_tilt,A_tilt) / torch.sqrt(torch.tensor(H))).reshape(N,-1,1) # M_attn -> (N,16,1) we need to use bmm so have to do this
    M_attn_tilt = torch.nn.functional.softmax(M_attn,dim = 1) # (N,16,1)
    attn = torch.bmm(A_tilt,M_attn_tilt).reshape(N,H)
    attn_weights = M_attn_tilt.reshape(N,4,4)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def attention_forward(x, A, Wx, Wh, Wattn, b):
    ...
    f_{att}(a_i, h_{tÃƒÆ’Ã‚Â¢Ãƒâ€¹Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢1}) equals to the scaled dot product of a_i and h_{t-1}.
