...
class ProposalModule(nn.Module):
  def __init__(self, in_dim, hidden_dim=256, num_anchors=9, drop_ratio=0.3):
    ...
    self.pred_layer = nn.Sequential(
            nn.Conv2d(in_dim,hidden_dim,3,padding = 1), # 7 - 3 + 2 + 1 = 7
            nn.Dropout(p = self.p),
            nn.LeakyReLU(),
  ...
  def forward(self, features, pos_anchor_coord=None, \
    ...
    ############################################################################
    # Replace "pass" statement with your code
    output = self.pred_layer(features) # (B,A * 6,7,7)
    B,in_dim,H_,W_ = features.shape
    print(output.shape)
    output = output.view(B,self.num_anchors,6,H_,W_)
    ...
    if mode == 'train':
        ...
        print(offsets.shape)
        print(pos_anchor_coord.shape)
        proposals = GenerateProposal(pos_anchor_coord.view(M, 1, 1, 1, 4), offsets.view(M, 1, 1, 1, 4), method='FasterRCNN')
...
class RPN(nn.Module):
  ...
  def forward(self, images, bboxes, output_mode='loss'):
    ...
    # Replace "pass" statement with your code
    B = images.shape[0]
    features = self.feat_extractor(images) # i)
    ...
    pos_anchor_idx, neg_anchor_idx, GT_conf_scores, GT_offsets, GT_class,activated_anc_coord, negative_anc_coord = ReferenceOnActivatedAnchors(anc_list, bboxes, grid_list, iou_mat,neg_thresh=0.3, method='FasterRCNN')
    ...
    total_loss = w_conf * conf_loss + w_reg * reg_loss
    # conf_scores只返回pos_conf
    conf_scores = conf_scores[:M,:][:,0].unsqueeze(1)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
  ...
  def inference(self, images, thresh=0.5, nms_thresh=0.7, mode='RPN'):
    ...
    with torch.no_grad():
        ...
        conf_probs = torch.sigmoid(conf_scores[:, :, 0, :, :]).squeeze() # B,A,2,H',W'-->B,A,H',W' (B,A,7,7)
        ...
        for b in range(B):
          ...
          masked = conf_probs[b][index].view(-1) # P_i
...
class TwoStageDetector(nn.Module):
  ...
  def forward(self, images, bboxes):
    ...
    # Replace "pass" statement with your code
    """
        images, bboxes, output_mode='loss'
        return total_loss, conf_scores, proposals, features, GT_class, pos_anchor_idx, anc_per_img
    """
    ...
    cls_loss = nn.functional.cross_entropy(class_prob, GT_class, reduction='sum') * 1. / images.shape[0]
  ...
  def inference(self, images, thresh=0.5, nms_thresh=0.7):
    ...
    B, _, H, W = features.shape # h,w = 7,7
