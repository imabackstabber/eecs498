...
class Conv(object):
  ...
  def forward(x, w, b, conv_param):
    ...
    out = torch.zeros(N,F,H_out,W_out,device = x.device,dtype = x.dtype) # 标注上device会规避无所谓的错误
    x_padding = torch.nn.functional.pad(x,(pad,pad,pad,pad),mode = 'constant')
    for img in range(N):
        for f in range(F):
            for i in range(H_out):
                for j in range(W_out):
                    out[img,f,i,j] = torch.sum(x_padding[img,:,i*stride:i*stride+HH,j*stride:j*stride+WW]*w[f,:,:,:]) + b[f]
    # 注意到conv会是Hadamard乘积哦
  ...
  def backward(dout, cache):
    ...
    dx = torch.zeros_like(x_padding)
    dw = torch.zeros_like(w)
    db = torch.zeros_like(b)
    for img in range(N):
        for f in range(F):
            db[f] += torch.sum(dout[img,f,:,:])
            for i in range(H_out):
                for j in range(W_out):
                    dw[f,:,:,:] += dout[img,f,i,j] * x_padding[img,:,i*stride:i*stride+HH,j*stride:j*stride+WW]
                    dx[img,:,i*stride:i*stride+HH,j*stride:j*stride+WW] += dout[img,f,i,j] * w[f,:,:,:]
    # 仅仅取没有padding的原部分
  ...
  def forward(x, pool_param):
    ...
    for img in range(N):
        for c in range(C):
            for i in range(H_out):
                for j in range(W_out):
                    out[img,c,i,j] = torch.max(x[img,c,i*stride:i*stride+pool_height,j*stride:j*stride+pool_width])
  ...
  def backward(dout, cache):
    ...
    for img in range(N):
        for c in range(C):
            for i in range(H_out):
                for j in range(W_out):
                    img_patch = x[img,c,i*stride:i*stride+pool_height,j*stride:j*stride+pool_width]
                    # 受限制于argmax的返回值，从0开始的标量
                    idx = torch.argmax(img_patch)
                    row = idx // pool_width
                    col = idx % pool_width
                    dx[img,c,i*stride:i*stride+pool_height,j*stride:j*stride+pool_width][row,col] = dout[img,c,i,j]
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    return dx
...
class ThreeLayerConvNet(object):
  ...
  def __init__(self, input_dims=(3, 32, 32), num_filters=32, filter_size=7,
    ...
    self.params['W2'] = torch.randn(in_dim,hidden_dim,device = device,dtype = dtype) * weight_scale
  ...
  def loss(self, X, y=None):
    ...
    ############################################################################
    # Replace "pass" statement with your code
    CRP,cache_CRP = Conv_ReLU_Pool.forward(X,W1,b1,conv_param,pool_param)
    LR,cache_LR = Linear_ReLU.forward(CRP,W2,b2)
    L,cache_L = Linear.forward(LR,W3,b3)
    scores = L
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    loss += self.reg * (torch.sum(torch.square(W3)) + torch.sum(torch.square(W2) + torch.sum(torch.square(W1))))
    
    dL,dW3,grads['b3'] = Linear.backward(dout,cache_L)
    dLR,dW2,grads['b2'] = Linear_ReLU.backward(dL,cache_LR)
    _,dW1,grads['b1'] = Conv_ReLU_Pool.backward(dLR,cache_CRP)
...
class DeepConvNet(object):
  ...
  def __init__(self, input_dims=(3, 32, 32),
    ...
    # Replace "pass" statement with your code
    C,H,W = input_dims
    num_max_pool = len(self.max_pools)
    cat_flr = [C]
    cat_flr += num_filters
    for i in range(len(num_filters)):
        if weight_scale == 'kaiming':
            self.params['W'+str(i+1)] = kaiming_initializer(cat_flr[i], num_filters[i], K=3, relu=True, device=device,
                        dtype=dtype)
        else:
            self.params['W'+str(i+1)] = weight_scale * torch.randn(num_filters[i],cat_flr[i],3,3,device = device,dtype = dtype)
    ...
    # last layer
    # 因为卷积后会跟上一个池化，所以不能直接算
    final_input_dim = num_filters[-1] * (H // (2 ** num_max_pool))* (W // (2 ** num_max_pool))
    if weight_scale == 'kaiming':
        self.params['W'+str(self.num_layers)] = kaiming_initializer(final_input_dim,num_classes, K=None, relu=False, device=device,
                        dtype=dtype) # 最后一层一定不能加ReLU
    else:
        self.params['W'+str(self.num_layers)] = weight_scale * torch.randn(final_input_dim,num_classes,device = device,dtype = dtype)
  ...
  def loss(self, X, y=None):
    ...
    for i in range(self.num_layers-1):
        ...
        if i in max_pool_set:
            if self.batchnorm:
              # 采用Conv_Bn_ReLU_Pool
              gamma = self.params['gamma'+str(i+1)]
              beta = self.params['beta'+str(i+1)]
              bn_param = self.bn_params[i]
              h_out,cache = Conv_BatchNorm_ReLU_Pool.forward(h_out,W,b,gamma,beta,conv_param,bn_param,pool_param)
              caches.append(cache)
            else:
              # 采用Conv_ReLU_Pool
    ...
    dout,dw,grads['b'+str(self.num_layers)] = Linear.backward(dout,caches.pop())
    grads['W'+str(self.num_layers)] = dw + 2 * self.reg * self.params['W'+str(self.num_layers)]
    
    for i in range(self.num_layers - 1):
        if self.num_layers-2-i in max_pool_set:
            if self.batchnorm:
              dout,dw,db,dgamma,dbeta = Conv_BatchNorm_ReLU_Pool.backward(dout, caches.pop())
              ...
              # use CRP
              dout,dw,db = Conv_ReLU_Pool.backward(dout,caches.pop())
              grads['W'+str(self.num_layers-1-i)] = dw + 2 * self.reg * self.params['W'+str(self.num_layers-1-i)]
              grads['b'+str(self.num_layers-1-i)] = db
        else:
            if self.batchnorm:
...
def find_overfit_parameters():
  ...
  weight_scale = 2e-1
  learning_rate = 1e-3
  print('training with ws:{},lr:{}'.format(weight_scale,learning_rate))
  ############################################################################
  #                             END OF YOUR CODE                             #
  ############################################################################
...
def create_convolutional_solver_instance(data_dict, dtype, device):
  ...
  solver = Solver(model, data_dict,
                    print_every=50, num_epochs=5, batch_size=128,
                    update_rule=adam,
                    # lr_decay = 0.99,
                    optim_config={
                      'learning_rate': lr,
...
def kaiming_initializer(Din, Dout, K=None, relu=True, device='cpu',
  ...
  if K is None:
    ...
    # Replace "pass" statement with your code
    fan_in = Din
    ws_ts = torch.tensor(gain / fan_in)
    weight = torch.randn(Din,Dout,dtype = dtype,device = device) * torch.sqrt(ws_ts)
    ###########################################################################
    #                            END OF YOUR CODE                             #
    ###########################################################################
  else:
    ...
    fan_in = Din * K * K
    ws_ts = torch.tensor(gain / fan_in)
    weight = torch.randn(Dout,Din,K,K,dtype = dtype,device = device) * torch.sqrt(ws_ts) 
    ###########################################################################
    #                            END OF YOUR CODE                             #
    ###########################################################################
  return weight
...
class BatchNorm(object):
  ...
  def forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
      ...
      #######################################################################
      # Replace "pass" statement with your code
      cache = {"gamma": gamma, "eps": eps, "beta": beta, "data":x, "mode":"train"}
      mean = x.mean(dim = 0)
      ...
      #######################################################################
      # Replace "pass" statement with your code
      cache = {"gamma": gamma, "eps":eps, "beta": beta, "data":x,"mode": "test", "runing_mean": running_mean, "running_var": running_var}
  ...
  def backward(dout, cache):
    ...
    if mode == 'train':
      ...
      eps = cache['eps']
      dbeta = 1. * dout.sum(dim=0)
      dgamma = torch.sum((x-r_mean)/((r_val + eps) ** 0.5) * dout, dim=0)
