...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    scores,cache_2 = Linear.forward(hidden,self.params['W2'],self.params['b2']) # 注意到只有隐藏层才使用了ReLU
    ###########################################################################
    #                            END OF YOUR CODE                             #
    ###########################################################################
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    loss,dout = softmax_loss(scores,y) # 拿到上游梯度
    loss += self.reg * torch.sum(self.params['W1'] ** 2) + self.reg * torch.sum(self.params['W2'] ** 2)
    dhidden,grads['W2'],grads['b2'] = Linear.backward(dout,cache_2)
    _,grads['W1'],grads['b1'] = Linear_ReLU.backward(dhidden,cache_1)
  ...
  def loss(self, X, y=None):
    ...
    for i in range(self.num_layers - 1):
        ...
        if self.use_dropout:
          ...
          drop_caches.append(dropped)
        
    w = self.params['W'+str(self.num_layers)]
    b = self.params['b'+str(self.num_layers)]
    h_out,cache = Linear.forward(h_out,w,b) # 注意到最后一层不会再有ReLU
    caches.append(cache)
    scores = h_out
    ############################################################################
    #                             END OF YOUR CODE                             #
    ...
    dout,dw,db = Linear.backward(dout,caches.pop()) # 弹出 并且 return
    grads['W' + str(self.num_layers)] = dw + 2 * self.reg * self.params['W' + str(self.num_layers)] # 不要忘了正则项
    grads['b' + str(self.num_layers)] = db
    
    for i in range(0,self.num_layers-1):
        # Linear->ReLU->Dropout
        if self.use_dropout:
            dout = Dropout.backward(dout,drop_caches.pop())
...
def create_solver_instance(data_dict, dtype, device):
  ...
  # Replace "pass" statement with your code
  lr = 1e-1
  solver = Solver(model,data_dict,optim_config={'learning_rate': 1e-1},lr_decay=0.95,num_epochs=20,device = device) 
  print('lr:{}'.format(lr))
  # 一定要加上device = device
  ##############################################################################
  #                             END OF YOUR CODE                               #
...
def get_three_layer_network_params():
  ...
  # Replace "pass" statement with your code
  learning_rate = 5e-1
  weight_scale = 7e-2 # why this param matters much?
  ############################################################################
  #                             END OF YOUR CODE                             #
  ############################################################################
...
def get_five_layer_network_params():
  ...
  learning_rate = 3e-1
  weight_scale = 1e-1
  print("training 5 layers with lr:{} and ws:{}".format(learning_rate,weight_scale))
  ############################################################################
  #                             END OF YOUR CODE                             #
  ############################################################################
  return weight_scale, learning_rate
