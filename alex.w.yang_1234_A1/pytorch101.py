...
def multiples_of_ten(start, stop):
  ...
  # Replace "pass" statement with your code
  x = []
  tmp = torch.arange(start,stop,dtype = torch.float64)
  for i in tmp:
      if i % 10 == 0:
        x.append(i)
...
def make_one_hot(x):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  mx = torch.max(torch.tensor(x)) + 1
  y = torch.zeros(len(x),mx)
...
def normalize_columns(x):
  ...
  tmp = (x-miu) ** 2
  delta = torch.sqrt(torch.sum(tmp,dim = 0)/(M-1))
  y = (x - miu)/delta
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
