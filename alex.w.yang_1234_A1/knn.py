...
def compute_distances_two_loops(x_train, x_test):
  ...
  for i in range(num_train):
      for j in range(num_test):
          dists[i,j] = torch.sqrt(torch.sum(torch.square(x_train_flatten[i] - x_test_flatten[j])))
  print('own two loops')
...
def compute_distances_one_loop(x_train, x_test):
  ...
  x_test_flatten = x_test.reshape(num_test,-1)
  for i in range(num_train):
      dists[i,:] = torch.sqrt(torch.sum(torch.square(x_train_flatten[i] - x_test_flatten),dim=1))
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_no_loops(x_train, x_test):
  ...
  dists = -2 * torch.mm(x_train_flat,x_test_flat.t()) + torch.sum(x_test_flat**2,dim=1).reshape(1,-1) + torch.sum(x_train_flat**2,dim=1).reshape(-1,1)
#   dists = -2 * torch.mm(x_test_flat,x_train_flat.t()) + torch.sum(x_train_flat**2,dim=1).reshape(1,-1) + torch.sum(x_test_flat**2,dim=1).reshape(-1,1)
#   dists = dists.t()
...
def predict_labels(dists, y_train, k=1):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  _,tmp = torch.topk(dists,k,dim = 0,largest = False)
  for i in range(num_test):
      idx = y_train[tmp[:,i]]
      y_pred[i],_ = torch.mode(idx)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return y_pred
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  for k in k_choices:
      ...
      for i in range(num_folds):
          ...
          k_to_accuracies[k].append(knn.check_accuracy(x_val,y_val,k = k))
  ##############################################################################
  #                            END OF YOUR CODE                                #
  ##############################################################################
...
def knn_get_best_k(k_to_accuracies):
  ...
  for k in k_to_accuracies.keys():
      kth_acc = statistics.mean(k_to_accuracies[k])
      if kth_acc > acc_best:
          acc_best = kth_acc
          best_k = k
  ##############################################################################
