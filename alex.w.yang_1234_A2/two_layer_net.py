...
def nn_forward_backward(params, X, y=None, reg=0.0):
    ...
    expsum = torch.sum(exp,dim = 1)
    correct = exp[torch.arange(N),y]
    correct_prob = correct / expsum
    loss += -torch.sum(torch.log(correct_prob))/N
    loss += reg * (torch.sum(W2 * W2) + torch.sum(W1*W1)) # 对于每一层都如此
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    factor = torch.div(exp,expsum.view(-1,1))
    factor[torch.arange(N),y] = -1 * (expsum - correct) / expsum
    
    grads['b2'] = torch.sum(factor,dim = 0)/N
    grads['W2'] = torch.mm(h1.t(),factor)/N + 2 * reg * W2
    
    hidden = factor.mm(W2.t())
    hidden[h1 == 0] = 0

    grads['b1'] = torch.sum(hidden,dim = 0)/N
    grads['W1'] = torch.mm(X.t(),hidden)/N + 2 * reg * W1
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_train(params, loss_func, pred_func, X, y, X_val, y_val,
  ...
  for it in range(num_iters):
    ...
    if it % iterations_per_epoch == 0:
      ...
      val_acc = (y_val_pred == y_val).float().mean().item() # gotta learn that elegant shit
...
def nn_get_search_params():
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  lr = [1e-1,3e-1,5e-1,7e-1,1e0]
  hs = [64,128,256,512]
  rs = [1e-5,3e-5,5e-5,7e-5,1e-4]
  lrc = [1.0,0.95,0.90,0.85]
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
...
def find_best_net(data_dict, get_param_set_fn):
  ...
  lr,hs,rs,lrc = get_param_set_fn()
  for l in lr:
      for h in hs:
        for r in rs:
            for lc in lrc:
                print("training with lr:{},hs:{},rs:{},lc:{}".format(l,h,r,lc))
                model =TwoLayerNet(32 * 32 * 3,h,10,device = data_dict['X_train'].device)
                stat = model.train(data_dict['X_train'], data_dict['y_train'], data_dict['X_val'], data_dict['y_val'],
                            learning_rate=l, learning_rate_decay=lc,
                            reg=r, num_iters=2000,
                            batch_size=200, verbose=False)
                y_pred = model.predict(data_dict['X_val'])
                val_acc = (y_pred == data_dict['y_val']).float().mean().item()
